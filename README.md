...
- **Отчёт агрегации + CSV (ур. 0)** — создаются рядом с исходным JSON файлы `*_agg_report.json` и `*_lv0.csv`. Нужны поля **productGroup** (по умолчанию `bio`) и **participantId** (обязательно с формы). В JSON отчёта поле `sntins` содержит компактные коды **GTIN (14 цифр) + серия (13 символов)** без криптохвоста; в CSV — **полные** штрихкоды из исходного JSON.
